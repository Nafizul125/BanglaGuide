    ...
    if request.method == 'POST':
        ...
        if action == 'update_dates':
            ...
            try:
                ...
                return JsonResponse({'status': 'not_found', 'count': len(cart)})
            new_cart = [it for it in cart if not (it.get('type') == item_type and int(it.get('id', -1)) == item_id)]
            request.session['cart'] = new_cart
            request.session.modified = True
            return JsonResponse({'status': 'removed', 'count': len(new_cart)})
